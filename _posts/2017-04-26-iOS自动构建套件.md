...
4. 注册[**fir.im**](https://fir.im/)账号，生成并获取**API Token**（不收费，不过要手机号码接收短信验证码）![](https://raw.githubusercontent.com/KeithLier/KeithLier.github.io/master/img/10_fir_APITOKEN.png)
...
3. 项目基础配置。选择苹果图标，Xcode版本。点击**创建**![](https://raw.githubusercontent.com/KeithLier/KeithLier.github.io/master/img/1_project_base_config%402x.png)
...
6. 设置。找到上传证书 & Provisioning Profile，上传**准备工作6**准备的描述文件和证书。![](https://raw.githubusercontent.com/KeithLier/KeithLier.github.io/master/img/4_provisioningProfile_Cert.png)



#### 构建
1. 自动构建。选择"构建列表"，一般等几秒钟就会出现你配置仓库的分支信息了
   ![](https://raw.githubusercontent.com/KeithLier/KeithLier.github.io/master/img/6_auto_build.png)

2. 手动构建。如果你嫌自动构建读取分支信息慢，点击手动构建，选择自己想构建的分支。
   ![](https://raw.githubusercontent.com/KeithLier/KeithLier.github.io/master/img/7_manual_select_branch.png)

3. 构建过程。点击构建当前构建条目（如果状态是**准备资源**，则需要等待几秒钟。然后自动会变为**运行中**。），即可看到如下图的构建流程。通常编译阶段会比较容易发生错误信息。按照错误提示修改项目，然后提交到Coding。**flow.ci**会自动同步代码库并运行构建。![](https://raw.githubusercontent.com/KeithLier/KeithLier.github.io/master/img/8_build_error.png)

4. 构建后。发布构建的APP到fir.im平台。首先需要点击**工作流**中左边的"+"号并搜索fir，添加**fir.im上传插件**。在**FIR_API_TOKEN**框中填入**准备工作4**获取的**API Token**。如果一切顺利的话，你会在**fir.im - 我的应用**里面看到构建成功的APP包了。赶紧分享你的APP下载二维码吧。![](https://raw.githubusercontent.com/KeithLier/KeithLier.github.io/master/img/9_flow_update.png)
